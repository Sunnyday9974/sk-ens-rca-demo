...
with col2:
    ...
    if submit_button:
        ...
        # --- [Step 1] 진행 연출 ---
        step1_placeholder.info("⏳ [Step 1] Neo4j 그래프 데이터베이스 접속 중... 알람-고장 모드 온톨로지 토폴로지 구조 분석")
        time.sleep(1.2)
        step1_placeholder.success("✔ [Step 1 완료] Neo4j 지식 그래프 탐색을 통해 설비 간 인과관계 메커니즘 도출 완료")
        
        # --- [Step 2] 진행 연출 ---
        step2_placeholder.info("⏳ [Step 2] Pinecone 벡터 데이터베이스 검색 중... 과거 10년 치 RCA 이력 및 기술 매뉴얼 매칭")
        time.sleep(1.2)
        step2_placeholder.success("✔ [Step 2 완료] Pinecone 고차원 임베딩 검색을 통해 실무 규격 및 유사 고장 사례 정합성 매칭 완료")
        
        # --- [Step 3] 진행 및 n8n 실제 통신 ---
        # 이 부분이 화면을 터치지 않도록 스피너 밖으로 완전히 독립시켰습니다.
        step3_placeholder.info("⏳ [Step 3] AI 에이전트 컨텍스트 융합 및 정식 엔지니어링 리포트 생성 중...")
        
        # n8n Webhook 연동 통신 호출
        ...
        try:
            # 안전하게 포스트 요청 발송 (네트워크 대기 중에도 화면이 유지됩니다)
            response = requests.post(n8n_url, json=payload, timeout=60)
            
            if response.status_code == 200:
                # 성공 시에만 3단계 상태를 완료로 업데이트
                step3_placeholder.success("✔ [Step 3 완료] LLM 기반 종합 지식 컨텍스트 동시 공조 가이드 수립 완료")
                ...
                step3_placeholder.error(f"❌ Step 3 실패: n8n 서버 연동 오류 (코드 {response.status_code})")
